...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        for(i=0; i<bn; i++)
        {
            tmp = l1_distance(a[j].data, b[i].data, a[j].n);
            if(tmp<distance)
            {
                distance = tmp;
                bind = i;
...
point project_point(matrix H, point p)
{
    ...
    matrix tmp = matrix_mult_matrix(H,c);
    point q = make_point(tmp.data[0][0]/tmp.data[2][0],tmp.data[1][0]/tmp.data[2][0]);
    free_matrix(c);
    free_matrix(tmp);
    return q;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for(int i=0; i<n; i++)
    {
        a_projected = project_point(H,m[i].p);
        distance = point_distance(a_projected, m[i].q);
        // Here we assume that all items of m before count are already inliers
        // so we replace m[count] with an inlier
        if(distance < thresh)
        {
            tmp = m[i];
            m[i] = m[count];
...
matrix compute_homography(match *matches, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        M.data[i*2+1][4] = y;
        M.data[i*2+1][5] = 1;
        M.data[i*2+1][6] = -1*x*yp;
        M.data[i*2+1][7] = -1*y*yp;
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for(int i=0; i<k; i++)
    {
        ...
        curr_inliers = model_inliers(H, m, n, thresh);
        if(curr_inliers > best)
        {
            best = curr_inliers;
            Hb = compute_homography(m, curr_inliers);
            if(best > cutoff)
            {
                free_matrix(H);
                return Hb;
